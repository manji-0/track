...
fn test_handle_list_no_output_errors() {
    ...
    let t1 = task_service.create_task("Active 1", None, None, None).unwrap();
    let t2 = task_service.create_task("Active 2", None, None, None).unwrap();
...
fn test_handle_info_no_output_errors() {
    ...
    let task = task_service.create_task("Task", Some("Description"), Some("INFO-1"), Some("http://url")).unwrap();
    ...
    link_service.add_link(task.id, "http://example.com", Some("Example")).unwrap();
...
fn test_handle_ticket_links() {
    ...
    assert_eq!(updated_task.ticket_url.as_deref(), Some("http://ticket.com"));
}
