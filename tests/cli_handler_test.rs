...
use track::services::{LinkService, RepoService, ScrapService, TaskService, TodoService, WorktreeService};
...
fn test_handle_new_creates_and_switches_task() {
    ...
    let current_id = db.get_current_task_id().unwrap().expect("Should have active task");
...
fn test_handle_archive_clean_worktree() {
    ...
    let task = task_service.create_task("Task", None, Some("PROJ-999"), None).unwrap();
    ...
    worktree_service.add_worktree(task.id, &repo_path, None, Some("PROJ-999"), None, true).unwrap();
...
fn test_handle_sync() {
    ...
    let task = task_service.create_task("Task Sync", None, Some("SYNC-123"), None).unwrap();
...
fn test_handle_sync_repo_not_found() {
    ...
    let task = task_service.create_task("Task", None, Some("SYNC-404"), None).unwrap();
...
fn test_handle_sync_branch_already_exists() {
    ...
    let task = task_service.create_task("Task", None, Some("SYNC-200"), None).unwrap();
    ...
    std::process::Command::new("git").args(["-C", &repo_path, "branch", "task/SYNC-200"]).output().unwrap();
...
fn test_handle_sync_worktree_already_exists() {
    ...
    let task = task_service.create_task("Task", None, Some("SYNC-300"), None).unwrap();
    ...
    worktree_service.add_worktree(task.id, &repo_path, None, Some("SYNC-300"), Some(todo.id), false).unwrap();
...
fn test_handle_sync_skip_done_todos() {
    ...
    let task = task_service.create_task("Task", None, Some("SYNC-400"), None).unwrap();
