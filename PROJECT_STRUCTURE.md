...
Install with [Skills CLI](https://github.com/vercel-labs/skills): `npx skills add ./skills -s track -s track-task-setup -s track-task-execute -s track-advanced -a cursor -a claude-code -a codex -y`. Plugin manifests: `.claude-plugin/`, `.codex-plugin/`, `.agents/plugins/`. Validate: `python3 scripts/validate_package.py`. See `skills/INSTALL.md`.
